...
-- by the Supabase security advisor after the first three migrations landed.
--
...
-- Same reasoning; nothing should call this outside its triggers.
...
-- is_admin stays callable: SECURITY INVOKER, and it only reads the caller's own
-- JWT, so a client asking whether it is an admin learns nothing it did not hold.
